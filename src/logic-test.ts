...
function addProppsToProduct(product: any, props_id: number, value: string) {
    ...
    const result = category_props.find((item) => item.category_id == product.category_id && item.properties_id == props_id);
    if (result) {
        products_props.push({
...
function categoryPropsValueUpdate(category_id: number, props_id: number, value: string) {
    ...
    const idx = category_props.findIndex((item) => item.category_id == category_id && item.properties_id == props_id);
    console.log("idx", idx);
...
const generalObject = {
    ...categories[0],
    properties: category_props.map((item) => {
        if (item.category_id == categories[0].id) {
            const property = properties.find(elem => elem.id == item.properties_id)
            return {
                name: property.name,
                values: item.values
            }
